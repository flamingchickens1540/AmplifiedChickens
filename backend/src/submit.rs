...
pub async fn admin_sse_connect(
    ...
    let mut upstream = state.sse_stream.lock().await;

    let (tx, rx) = watch::channel(Ok(Event::default())); // tx is the upstream, while rx is the downstream
...
pub enum SseReturn {
    ...
    TeamMatchScouted { scout_id: String, team_key: String, match_key: String }
}
...
pub async fn submit_team_match(
    ...
    match upstream {
        Some(stream) => {
            let team_match = SseReturn::TeamMatchScouted { scout_id: form.scout_id, team_key: form.team_key, match_key: form.match_key };

            stream.send(Ok(team_match));
        },
        None => {
            error!("None fatal: Team Match submitted without a registered admin present");
        }
    }
